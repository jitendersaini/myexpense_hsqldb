...
public class ChartController {
	...
	public String loadForm(Model model, HttpServletRequest request) {
		...
		Map<Short, String> grouping = new HashMap<Short, String>();
	...
	public String loadPieChart(Model model, HttpServletRequest request,
		...
		for (Expense expense : list) {
			...
			if (StringUtils.isEmpty(grouping) || grouping.equalsIgnoreCase("1")) {
				if (null != map.get(expense.getCategory().getId())) {
					map.put(expense.getCategory().getId().toString(), (map
							.get(expense.getCategory().getId()) + expense
							.getExpenseValue()));
				} else {
					map.put(expense.getCategory().getId().toString(),
							expense.getExpenseValue());
				}
				catIdValue.put(expense.getCategory().getId().toString(),
						expense.getCategory().getCategoryName());
			} else {
				if (null != map.get(expense.getPaymentMode().getId())) {
					map.put(expense.getPaymentMode().getId().toString(), (map
							.get(expense.getPaymentMode().getId()) + expense
							.getExpenseValue()));
				} else {
					map.put(expense.getPaymentMode().getId().toString(),
							expense.getExpenseValue());
				}
				catIdValue.put(expense.getPaymentMode().getId().toString(),
						expense.getPaymentMode().getPaymentMode());
			}
		}
