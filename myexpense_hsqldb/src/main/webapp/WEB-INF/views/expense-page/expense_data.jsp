...
<table id="exptable" class="display">
	...
	<tbody>
		<c:forEach items="${list}" var="v" varStatus="status">
			...
			<td><input type="hidden" class="clazzPM" name="hidPaymentMode" value="${v.paymentMode.id}"/>${v.paymentMode.paymentMode}</td>
			<td align="center"><fmt:formatDate value="${v.expenseDate}"
					type="date" pattern="MMM dd, yyyy" /><input type="hidden" name="hidUserid" value="${v.usersByCreatedBy.id}"/></td>
			<td>${v.usersByCreatedBy.name}</td>
